...
class ScrutinyBoardMeetingDocumentRepository extends BaseRepository
{
    ...
    public function update($meeting)
    {
        ...
        $currentIds = $this->getRecordIds($meeting);
        $updatingIds = [];
        ...
        if (is_array($ids) && count($ids) > 0) {
            ...
            foreach ($ids as $key => $id) {
                if ($id == "" || !in_array($id, $currentIds)) {
                    $record = new ScrutinyBoardMeetingDocument();
                    $record->scrutiny_board_meeting_id = $meeting->id;
                    ...
                    $record = ScrutinyBoardMeetingDocument::query()->find($id);
                    $record->document_name = $documentDetails[$key];
        ...
        if (count($notUpdatingIds) > 0) {
            $meeting->documents()->whereIn("id", $notUpdatingIds)->delete();
    ...
    public function getRecordIds($meeting): array
    {
     ...
     * @return mixed
     */
    public function triggerDownloadDocument($meeting, $documentId)
    {
        $record = ScrutinyBoardMeetingDocument::query()->find($documentId);

        if ($record) {
            $docName = str_replace(".", " ", $meeting->name . "-" . $meeting->id . "-") . $record->document_name;
