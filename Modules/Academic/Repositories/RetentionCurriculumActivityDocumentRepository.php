...
class RetentionCurriculumActivityDocumentRepository extends BaseRepository
{
    private string $upload_dir = "public/retention_curriculum_activity_documents/";
    ...
    public function update($rcActivity)
    {
        $ids = request()->post("doc_id");
        $documentDetails = request()->post("document_name");

        $currentIds = $this->getRecordIds($rcActivity);
        $updatingIds = [];
        ...
        if (is_array($ids) && count($ids) > 0) {
            ...
            foreach ($ids as $key => $id) {
                if ($id == "" || !in_array($id, $currentIds)) {
                    $record = new RetentionCurriculumActivityDocument();
                    $record->rc_activity_id = $rcActivity->id;
                    ...
                    $record = RetentionCurriculumActivityDocument::query()->find($id);
                    $record->document_name = $documentDetails[$key];
        ...
        if (count($notUpdatingIds) > 0) {
            $rcActivity->rcActivityDocuments()->whereIn("id", $notUpdatingIds)->delete();
    ...
    public function getRecordIds($rcActivity)
    {
        $documents = $rcActivity->rcActivityDocuments->toArray();
     ...
     * @return mixed
     */
    public function triggerDownloadDocument($rcActivity, $documentId)
    {
        $record = RetentionCurriculumActivityDocument::query()->find($documentId);

        if ($record) {
            $docName = str_replace(".", " ", $rcActivity->name . "-" . $rcActivity->id . "-") . $record->document_name;
