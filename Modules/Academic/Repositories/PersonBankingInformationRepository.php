...
use Modules\Academic\Entities\PersonBankingInformation;

class PersonBankingInformationRepository extends BaseRepository
{
    public function update($person)
    {
        ...
        if (is_array($ids) && count($ids) > 0) {
            ...
            foreach ($ids as $key => $id) {
                ...
                if ($id == "" || !in_array($id, $currentIds)) {
                    $records[] = new PersonBankingInformation($record);
                } else {
                    $person->bankingInfo()->where("id", $id)->update($record);
            ...
            if (count($records) > 0) {
                $person->bankingInfo()->saveMany($records);
            }
        }
        ...
        if (count($notUpdatingIds) > 0) {
            $person->bankingInfo()->whereIn("id", $notUpdatingIds)->delete();
        }
    }

    public function getRecordIds($person): array
    {
        $bankingInfo = $person->bankingInfo->toArray();
