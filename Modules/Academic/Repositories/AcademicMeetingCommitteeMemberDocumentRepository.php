...
class AcademicMeetingCommitteeMemberDocumentRepository extends BaseRepository
{
    private $upload_dir = "public/academic_meeting_committee_member_documents/";
    ...
    public function update($committeeMember)
    {
        ...
        $appointedFrom = request()->post("appointed_from");
        $appointedTill = request()->post("appointed_till");

        $currentIds = $this->getRecordIds($committeeMember);
        $updatingIds = [];
        ...
        if (is_array($ids) && count($ids) > 0) {
            ...
            foreach ($ids as $key => $id) {
                if ($id == "" || !in_array($id, $currentIds)) {
                    $record = new AcademicMeetingCommitteeMemberDocument();
                    $record->academic_meeting_committee_member_id = $committeeMember->id;
                    $record->document_name = $documentDetails[$key];
                    $record->appointed_from = $appointedFrom[$key];
                    $record->appointed_till = $appointedTill[$key];

                    $fileKey++;

                    if ($this->isValidFile($fileKey)) {
                        $fileName = preg_replace("/[^a-zA-Z0-9]+/", "", $_FILES[$this->formFileName]["name"][$fileKey]);
                    ...
                    $record = AcademicMeetingCommitteeMemberDocument::query()->find($id);
                    $record->document_name = $documentDetails[$key];
                    $record->appointed_from = $appointedFrom[$key];
                    $record->appointed_till = $appointedTill[$key];
        ...
        if (count($notUpdatingIds) > 0) {
            $committeeMember->documents()->whereIn("id", $notUpdatingIds)->delete();
    ...
    public function getRecordIds($committeeMember)
    {
        $documents = $committeeMember->documents->toArray();
     ...
     * @return mixed
     */
    public function triggerDownloadDocument($committeeMember, $documentId)
    {
        $record = AcademicMeetingCommitteeMemberDocument::query()->find($documentId);

        if ($record) {
            $docName = str_replace(".", " ", $committeeMember->name . "-" . $committeeMember->id . "-") . $record->document_name;
