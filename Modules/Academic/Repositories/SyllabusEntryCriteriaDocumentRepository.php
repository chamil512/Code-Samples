...
use Modules\Academic\Entities\SyllabusEntryCriteriaDocument;

class SyllabusEntryCriteriaDocumentRepository extends BaseRepository
{
    private $upload_dir = "public/syllabus_entry_criteria_documents/";
    ...
    public function update($entryCriteria)
    {
        $ids = request()->post("doc_id");
        $documentDetails = request()->post("document_name");

        $currentIds = $this->getRecordIds($entryCriteria);
        $updatingIds = [];
        ...
        if(is_array($ids) && count($ids)>0)
        {
            ...
            foreach ($ids as $key => $id)
            {
                ...
                {
                    $record = new SyllabusEntryCriteriaDocument();
                    $record->syllabus_entry_criteria_id = $entryCriteria->id;
                ...
                {
                    $record = SyllabusEntryCriteriaDocument::query()->find($id);
                    $record->document_name = $documentDetails[$key];
        ...
        {
            $entryCriteria->ecDocuments()->whereIn("id", $notUpdatingIds)->delete();
    ...
    public function getRecordIds($entryCriteria)
    {
        $entryCriteriaDocuments = $entryCriteria->ecDocuments->toArray();

        $ids = [];
        if(is_array($entryCriteriaDocuments) && count($entryCriteriaDocuments)>0)
        {
            foreach ($entryCriteriaDocuments as $record)
            {
                $ids[] = $record["id"];
            }
        }

        $this->currentRecords = $entryCriteriaDocuments;
    ...
    public function triggerDownloadDocument($entryCriteria, $documentId)
    {
        $record = SyllabusEntryCriteriaDocument::query()->find($documentId);

        if($record)
        {
            $docName = str_replace(".", " ", $entryCriteria->name . "-" . $entryCriteria->id . "-") . $record->document_name;
