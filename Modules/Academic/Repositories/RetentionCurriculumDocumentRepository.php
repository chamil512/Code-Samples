...
class RetentionCurriculumDocumentRepository extends BaseRepository
{
    private string $upload_dir = "public/retention_curriculum_documents/";
    ...
    public function update($curriculum)
    {
        ...
        $currentIds = $this->getRecordIds($curriculum);
        $updatingIds = [];
        ...
        if (is_array($ids) && count($ids) > 0) {
            ...
            foreach ($ids as $key => $id) {
                if ($id == "" || !in_array($id, $currentIds)) {
                    $record = new RetentionCurriculumDocument();
                    $record->retention_curriculum_id = $curriculum->id;
                    ...
                    $record = RetentionCurriculumDocument::query()->find($id);
                    $record->document_name = $documentDetails[$key];
        ...
        if (count($notUpdatingIds) > 0) {
            $curriculum->rcDocuments()->whereIn("id", $notUpdatingIds)->delete();
    ...
    public function getRecordIds($curriculum)
    {
        $documents = $curriculum->rcDocuments->toArray();
     ...
     * @return mixed
     */
    public function triggerDownloadDocument($curriculum, $documentId)
    {
        $record = RetentionCurriculumDocument::query()->find($documentId);

        if ($record) {
            $docName = str_replace(".", " ", $curriculum->name . "-" . $curriculum->id . "-") . $record->document_name;
